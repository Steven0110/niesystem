...
try{
    ...
    $sql = "SELECT s.folio, s.mano_obra, s.casetas, s.desplazamiento, s.partes_iva, s.cobro, s.observacion, s.garantia, r.semana FROM servicios_tecnico_trabajados s, reporte r WHERE r.idreporte=s.idreporte";
    $stm = $pdo->prepare( $sql );
    if( $stm->execute() ){
